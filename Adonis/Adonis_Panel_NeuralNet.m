classdef Adonis_Panel_NeuralNet < handle
    ...
    methods
        ...
        function obj = Adonis_Panel_NeuralNet(fig, Vpos, aNum)
        ...
        function st = checkState(obj)
            ...
            elseif obj.flagValue == 1 % when it's running and got data
                ...
                switch obj.curentState
                    ...
                    case 3
                        if sum(m(5)) == 1 % If the last 1 sec of emg are high, then it's wake
                            obj.curentState = 1;
                            updateState(1)
                        elseif sum(rR) >= 2 % if the ratio (delta higher) for 1 or more than 1 seconds it might be NREM again
                            obj.curentState = 2;
                            updateState(2)
                        end
                ...
                % INFRASALOW State decision with machine learning
                
                if obj.flagValue == 1
                    obj.infraEMG = [obj.infraEMG(2:end), emgNo];
                    tofeed = [obj.infraSmoothLine.YData, obj.infraEMG];
                    dec = Adonis_neuralNet(tofeed');
%                     tofeed = [obj.infraSmoothLine.YData, obj.infraEMG, obj.infraStateToFeed];
%                     dec = Adonis_neuralNetWithPreviousState(tofeed');
                    [~,declo] = max(dec);
                    switch declo
                        case 1
                            obj.infraState = 0; % nothing
                            obj.infraLabel.String = 'none';
                            obj.infraLabel.BackgroundColor = 'black';
                        case 2
                            obj.infraState = 1; % continuity
                            obj.infraLabel.String = 'Continuity';
                            obj.infraLabel.BackgroundColor = 'green';
                        case 3
                            obj.infraState = 2; % fragiltiy
                            obj.infraLabel.String = 'Fragility';
                            obj.infraLabel.BackgroundColor = 'red';
                    end
                end
                
                % st creation with specific order for each variable
                st = [obj.curentState; ratio; emgNo; obj.infraState; sigma; valfit(end)];
%                 obj.infraStateToFeed = [obj.infraStateToFeed(2:end), obj.infraState];
