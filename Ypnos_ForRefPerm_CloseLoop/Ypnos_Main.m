...
%% Settings and board connection
    ...
    function launchSett(~,~)
        ...
        h.panels = cell(1,h.nChip);
        if h.nChip <= 4
            positions = [0.67, 0.46,0.25,0.04];
        else
            positions = [0.67, 0.67-1*0.11, 0.67-2*0.11, 0.67-3*0.11,0.67-4*0.11,0.67-5*0.11,0.67-6*0.11,0.67-7*0.11];
        end
...
states = zeros(4, fLen*3600); %#ok<NASGU>

for i = 1:h.settings.nFiles
    for c = 1:h.nChip
        traces = zeros(ntraces(c)+1, fLen*3600*1000); %#ok<NASGU> % +1 traces for the analogs
